...
import numpy as np
from  create_scene import make_polygons, show_scene, create_plot, add_polygon_to_scene, save_polygons
...
def collision_space(car, obstacles, ax):
    car_coords = get_coords(car)
    cspace = []
    for o in obstacles:
        minkowski_sum_points = compute_minkowski_sum(car_coords, o)
        convex_hull = ConvexHull(minkowski_sum_points)
        minkowski_sum_polygon = minkowski_sum_points[convex_hull.vertices]
        translated = [(x - car.get_x(), y - car.get_y()) for x,y in minkowski_sum_polygon]
        cspace.append(translated)
        add_polygon_to_scene(translated, ax, False)
    print(cspace)
    cspacearray = np.array(cspace, dtype= object)
    save_polygons(cspacearray, '0initialp3')
...
if __name__ == '__main__':
    obstacles = np.load('ex2.npy', allow_pickle=True)
    ax = create_plot()
    for polygon in obstacles:
    ...
    while(True):
        ...
        car = patches.Rectangle((x,y),0.2,0.1,linewidth = 1, edgecolor = 'r', facecolor = 'blue', angle=0)
        if(check_car(car, obstacles)): break
    ax.add_patch(car)
    controller = CarController(ax, car, obstacles)
    #collision_space(car, obstacles, ax)
    show_scene(ax)
